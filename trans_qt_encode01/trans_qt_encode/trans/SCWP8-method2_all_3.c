...
float*** forward_transform(int N, int M, int depth) //N是宽 M是高
{
	...
	unsigned char *** ImgData8 = NULL;
	unsigned short *** ImgData10 = NULL;
	FILE *fp;
	int m, n;
	int num[3];
	int num2[3];
	...
	float*** ImgData = (float ***)malloc(sizeof(float **) * 96); //为了比较PSNR额外申请的数组，最终是不需要的，可以删除
	for (int i = 0; i < 96; i++)
	{
		ImgData[i] = (float**)malloc(sizeof(float*)* M);
		for (int j = 0; j < M; j++)
		{
			ImgData[i][j] = (float*)malloc(sizeof(float)* N);
		}
	}
	...
	if (depth == 8) {
		...
		if (ImgData8) {
			for (i = 0; i < 96; i++)
			{
				for (j = 0; j < M; j++)
				{
					for (k = 0; k < N; k++)
					{
						VideoData[i][j][k] = (float)ImgData8[i][j][k];
						ImgData[i][j][k] = VideoData[i][j][k];
					}
				}
			}
			/*内存释放*/
		...
		if (ImgData10) {
			for (i = 0; i < 96; i++)
			{
				for (j = 0; j < M; j++)
				{
					for (k = 0; k < N; k++)
					{
						VideoData[i][j][k] = (float)((double)ImgData10[i][j][k] / 4.0);
						ImgData[i][j][k] = VideoData[i][j][k];
					}
				}
			}
		...
		//FILE *fp3;
		//errno_t err;
		//if ((err = fopen_s(&fp3, "E:\\程序代码\\量化\\2021-01-15\\test_en_sub3d_sub2_sub\\Image.dat", "wb")) != 0){
		//	printf("打开数据失败！\n");
		//	system("pause");
		//	return 0;
		//}
		...
		//	{
		//		fwrite(&VideoData[i][j][0], sizeof(float), 1920, fp3);
		//	}
		//}
		//fclose(fp3);
	}
